...
if user_name:
    ...
    with menu[2]:
        ...
        if quiz_select == "거리: km ➡️ m 변환하기":
            ...
            ans = st.text_input("이 거리는 몇 m일까요? (숫자만 입력):", key="ans_km_m")
            if st.button("정답 확인", key="btn_km_m"):
                if ans == "6500":
                    st.success(f"🎉 정답입니다! 수의 크기가 1000배가 되는 원리를 잘 적용했어요!")
                    st.balloons()
                else: st.error("다시 생각해보세요. 큰 단위에서 작은 단위로 갈 때는 수의 크기가 1000배가 됩니다.")
                    
        # 퀴즈: m -> km
        elif quiz_select == "거리: m ➡️ km 변환하기":
            st.write("🔍 문제: 학교 운동장을 돌았더니 **4500 m**를 걸었습니다.")
            ans = st.text_input("이 거리는 몇 km일까요? (소수점을 정확히 찍어주세요):", key="ans_m_km")
            if st.button("정답 확인", key="btn_m_km"):
                if ans == "4.5":
                    st.success(f"🎉 정답입니다! 수의 크기가 1/1000배가 되어 자릿수가 이동했네요!")
                    st.balloons()
                else: st.error("m에서 km로 갈 때는 1000으로 나누어 자릿수를 왼쪽으로 옮겨야 합니다.")
                    
        # 퀴즈: kg -> g
        elif quiz_select == "무게: kg ➡️ g 변환하기":
            st.write("🔍 문제: 아기 고양이의 몸무게를 재어보니 **1.8 kg**이었습니다.")
            ans = st.text_input("이 몸무게는 몇 g일까요? (숫자만 입력):", key="ans_kg_g")
            if st.button("정답 확인", key="btn_kg_g"):
                if ans == "1800":
                    st.success(f"🎉 정답입니다! 1000배 큰 수로 멋지게 변환했습니다!")
                    st.balloons()
                else: st.error("kg 단위가 g 단위로 바뀌면 숫자는 1000배 커집니다.")
                    
        # 퀴즈: g -> kg
        elif quiz_select == "무게: g ➡️ kg 변환하기":
            st.write("🔍 문제: 택배로 보낼 상자의 무게가 **3200 g**이 나왔습니다.")
            ans = st.text_input("이 상자는 몇 kg일까요?", key="ans_g_kg")
            if st.button("정답 확인", key="btn_g_kg"):
                if ans == "3.2":
                    st.success(f"🎉 대단해요, 자릿수 줄이기 마스터 성공!")
                    st.balloons()
                else: st.error("g에서 kg으로 바뀔 때는 수의 크기가 1/1000배가 됩니다.")

        # 新 퀴즈: L -> mL
        elif quiz_select == "들이: L ➡️ mL 변환하기":
            st.write("🔍 문제: 과학 실험을 위해 비커에 물 **2.4 L**를 담았습니다.")
            ans = st.text_input("이 물의 양은 몇 mL일까요? (숫자만 입력):", key="ans_l_ml")
            if st.button("정답 확인", key="btn_l_ml"):
                if ans == "2400":
                    st.success(f"🎉 정답입니다, {user_name}님! 들이 단위도 1000배 커지는 원리를 정확히 파악했군요!")
                    st.balloons()
                else: st.error("L에서 mL로 갈 때도 수의 크기가 1000배가 되므로 소수점을 오른쪽으로 3칸 옮겨보세요.")

        # 新 퀴즈: mL -> L
        elif quiz_select == "들이: mL ➡️ L 변환하기":
            st.write("🔍 문제: 텀블러에 가득 채운 보리차의 양이 **750 mL**입니다.")
            ans = st.text_input("이 보리차의 양은 몇 L일까요? (소수점으로 표현해보세요):", key="ans_ml_l")
            if st.button("정답 확인", key="btn_ml_l"):
                if ans == "0.75":
                    st.success(f"🎉 정답입니다! 수의 크기가 1/1000배가 되어 소수점이 왼쪽으로 3칸 잘 이동했네요!")
                    st.balloons()
                else: st.error("아쉬워요! 750에서 소수점을 왼쪽으로 3칸 옮기면 얼마가 될지 다시 생각해 보세요.")
